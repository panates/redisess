...
export class Session {
  ...
  async get(key): Promise<any> {
    ...
    if (Array.isArray(key)) {
      keys = key.slice();
      for (const [i, k] of keys.entries()) keys[i] = '$' + k;
    ...
    const resp = await promisify.fromCallback(cb =>
      client.hmget(sessKey, keys, cb),
    );

    // Do type conversion
    for (const [i, v] of resp.entries()) resp[i] = await fromTyped(v);

    if (Array.isArray(key)) return resp;
  ...
  private _prepareUserData(arg0, arg1?): string[] {
    const makeTyped = v => {
      if (v instanceof Buffer) return 'b' + v.toString('base64');
