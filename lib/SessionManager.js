...
class SessionManager {
  ...
  constructor(client, appName, options) {
    ...
    this._ttl = Number(options.ttl) || (30 * 60);
  ...
  create(userId, options) {
    ...
    options = options || {};
    const ttl = options.ttl >= 0 ? options.ttl : this._ttl;
...
class Session {
  ...
  get expiresIn() {
    return this._expires - this.manager._now();
  }
  ...
  _prepareUpdate(pipeline) {
    ...
    this._expires = this._lastAccess + this._ttl;
