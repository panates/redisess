...
export function initSessionManagerTests(ctx: { redis }) {
    ...
    describe('SessionManager', function () {
        ...
        before(async function () {
            sm = new SessionManager(ctx.redis, {
                namespace: 'smtest',
                wipeInterval: 60000,
                additionalFields: ['peerIp', 'userAgent']
            });
            await sm.killAll();
            await promisify.fromCallback(cb => ctx.redis.script('flush', cb));
        });
        ...
        it('should constructor validate arguments', function () {
            ...
            // @ts-ignore
            new SessionManager(ctx.redis, 'myapp');
        });

        it('should set namespace while construct', function () {
            const sm = new SessionManager(ctx.redis, {namespace: 'abc'});
            assert.strictEqual(sm.namespace, 'abc');
        });

        it('should set ttl while construct', function () {
            const sm = new SessionManager(ctx.redis, {ttl: 60});
